// content.js - TAVY KOREA Olive Young Interactive Lightbox & Deep Scraper v16.0
// 1. Tự động tìm thẻ <a>/<li> bọc ngoài ảnh thumbnail -> Click mở Lightbox xem ảnh to -> Bấm Next liên tục bóc 30+ ảnh HD
...
if (typeof window.__TAVY_SCRAPER_LOADED__ === 'undefined') {
  ...
  const isJunkImage = (src, alt = '') => {
    ...
    return /\/display\/|\/event\/|\/banner\/|\/static\/|\/item\/|logo|icon|avatar|star_|btn_|badge|tag_|flag_|blank|loading|sprite/i.test(combined);
  };

  /**
   * BƯỚC 1: Lấy CHÍNH XÁC 3 Ảnh Sản Phẩm HD từ Đầu Trang
   */
  const pickProductImages = () => {
    ...
    if (ogImage && !isJunkImage(ogImage) && !ogImage.includes('gdasEditor')) {
      productUrls.push(getHighResUrl(ogImage));
    }

    mainNodes.forEach((img) => {
      const src = img.currentSrc || img.src || img.getAttribute('data-src') || img.getAttribute('data-original') || '';
      if (!src || isJunkImage(src) || src.includes('gdasEditor')) return;

      if (src.includes('/goods/') || (goodsNo && src.includes(goodsNo)) || /thumbnails/i.test(src)) {
  ...
  /**
   * BƯỚC 2: CÀO SÂU ĐA TRANG & ĐA SẮP XẾP GDAS API từ Server Olive Young
   */
  const fetchDeepMultiPageReviewPhotos = async (goodsNo) => {
    if (!goodsNo) return [];
    const photoUrls = [];
    const sorts = ['05', '01', '02'];
  ...
  /**
   * BƯỚC 3: CLICK VÀO THẺ THUMBNAIL/ẢNH ĐÁNH GIÁ ĐỂ MỞ LIGHTBOX POPUP & BẤM NEXT CHỤP TẤT CẢ ÁNH
   */
  const interactiveLightboxClickScrape = async () => {
    const photoUrls = [];
    try {
      const reviewThumbs = Array.from(document.querySelectorAll('#searchGdasList img, .gReviewList img, .rw-img-list img, [class*=gdas] img, .thumb_list img, .photo_list img, .img_box img') || []);
      const validThumbs = reviewThumbs.filter(img => {
        const src = img.currentSrc || img.src || img.getAttribute('data-src') || '';
        return src && src.startsWith('http') && !isJunkImage(src);
      });

      console.log(`⚡ [TAVY Scraper v16.0] Tìm thấy ${validThumbs.length} thumbnail ảnh đánh giá.`);
      ...
      if (validThumbs.length > 0) {
        ...
        // Lặp 25 lần bấm nút Next trong Popup xem ảnh to
        for (let i = 0; i < 25; i++) {
  ...
  const executeStepByStepScrape = async () => {
    ...
    showMiniToast('Step 4/5: Click thẻ <a> mở Lightbox & bấm Next...', 'info');
    window.scrollTo({ top: 2200, behavior: 'smooth' });
    await new Promise(r => setTimeout(r, 600));

    const lightboxPhotos = await interactiveLightboxClickScrape();

    // 5. Tổng hợp toàn bộ danh sách Ảnh Đánh Giá Thực Tế
    showMiniToast('Step 5/5: Tổng hợp bộ sưu tập 30+ Ảnh Đánh Giá...', 'info');
    const combinedCandidates = Array.from(new Set([...lightboxPhotos, ...deepReviewPhotos]));

    return {
      productImages: productImages.slice(0, 3),
      reviewCandidates: combinedCandidates.slice(0, 50)
    };
  };
